...
- [x] Modern UI Backend (NanoVG Integration)
- [x] Component Rendering and Event Handling
- [x] Config Annotation Framework
- [ ] **Config File Persistence**: Automatic JSON serialization using Gson in `ModuleManager.loadConfigs()` and `saveConfigs()`.
- [ ] **API Registry**: A global static Registry so multiple mods can seamlessly inject their tabs into one master Lucent Screen without overriding each other.

---
*Built with ❤️ for the Fabric Modding Community.*
