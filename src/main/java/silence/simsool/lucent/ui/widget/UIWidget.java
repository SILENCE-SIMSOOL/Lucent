...
public abstract class UIWidget {
	 ...
	 */
	public final void render(GuiGraphicsExtractor ctx, int mouseX, int mouseY, float delta) {
		if (!visible) return;
		hovered = enabled && isMouseOver(mouseX, mouseY);
		renderWidget(ctx, mouseX, mouseY, delta);
	}

	protected abstract void renderWidget(GuiGraphicsExtractor ctx, int mouseX, int mouseY, float delta);
