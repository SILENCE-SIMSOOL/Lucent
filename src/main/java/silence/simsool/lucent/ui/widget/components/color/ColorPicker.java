...
public class ColorPicker extends UIWidget {
	...
	private void applyInputValue() throws NumberFormatException {
		switch (activeInput) {
			case R -> {
				int[] rgb = hsvToRGB();
				rgb[0] = UAnimation.clamp((int) Float.parseFloat(inputBuffer), 0, 255);
				setFromRGB(rgb[0], rgb[1], rgb[2]);
			}
			case G -> {
				int[] rgb = hsvToRGB();
				rgb[1] = UAnimation.clamp((int) Float.parseFloat(inputBuffer), 0, 255);
				setFromRGB(rgb[0], rgb[1], rgb[2]);
			}
			case B -> {
				int[] rgb = hsvToRGB();
				rgb[2] = UAnimation.clamp((int) Float.parseFloat(inputBuffer), 0, 255);
				setFromRGB(rgb[0], rgb[1], rgb[2]);
			}
			case A -> {
				alpha = UAnimation.clamp(Float.parseFloat(inputBuffer) / 100f, 0, 1);
			}
			case HEX -> {
	...
	private int getCurrentARGB() {
		return UColor.fromHSVA(hue, saturation, value, alpha);
	}
