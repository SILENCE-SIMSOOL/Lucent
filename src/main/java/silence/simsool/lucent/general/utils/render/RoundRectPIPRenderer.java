...
public class RoundRectPIPRenderer extends PictureInPictureRenderer<RoundRectPIPRenderer.State> {
	...
	protected void renderToTexture(State state, PoseStack poseStack, net.minecraft.client.renderer.SubmitNodeCollector submitNodeCollector) {
		...
		try (ByteBufferBuilder byteBufferBuilder = new ByteBufferBuilder(1024)) {
			BufferBuilder builder = new BufferBuilder(byteBufferBuilder, PrimitiveTopology.QUADS, DefaultVertexFormat.POSITION_COLOR);
			...
			try (MeshData mesh = builder.buildOrThrow()) {
				...
				var buffer = RenderSystem.getDevice().createBuffer(
						() -> "Lucent Rounded Rectangle Vertex Buffer",
						GpuBuffer.USAGE_VERTEX,
						mesh.vertexBuffer()
				);
				var vertexBuffer = buffer.slice();
				var indexStorage = RenderSystem.getSequentialBuffer(mesh.drawState().primitiveTopology());
	...
	public static void submit(GuiGraphicsExtractor graphics, int x0, int y0, int x1, int y1, int topLeftColor, int topRightColor, int bottomRightColor, int bottomLeftColor, float topLeftRadius, float topRightRadius, float bottomRightRadius, float bottomLeftRadius, int outlineColor, float outlineWidth) {
		...
		graphics.guiRenderState.addPicturesInPictureState(new State(
