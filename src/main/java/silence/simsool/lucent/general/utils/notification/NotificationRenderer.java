	...
	public static void render(GuiGraphicsExtractor graphics) {
		...
		SkijaRenderer.draw(graphics, 0, 0, screenWidth, screenHeight, () -> {
			...
			for (int i = list.size() - 1; i >= 0; i--) {
				...
				float[] titleBounds = SkijaRenderer.wrappedTextBounds(n.getTitle(), maxTextW, Fonts.PRETENDARD_MEDIUM, 16.0f);
				float titleH = Math.max(16.0f, titleBounds[3] - titleBounds[1]);

				float msgH = 0.0f;
				if (n.hasMessage()) {
					float[] msgBounds = SkijaRenderer.wrappedTextBounds(n.getMessage(), maxTextW, Fonts.PRETENDARD, 12.0f);
				...
				SkijaRenderer.drawWrappedString(n.getTitle(), contentX, titleY, maxTextW, Fonts.PRETENDARD_MEDIUM, 16.0f, UColor.withAlpha(UIColors.PURE_WHITE, 225));

				// Message text
				if (n.hasMessage()) {
					float msgY = titleY + titleH + 8.0f;
					SkijaRenderer.drawWrappedString(n.getMessage(), contentX, msgY, maxTextW, Fonts.PRETENDARD, 12.0f, 0xCCD1D5DB);
				}
