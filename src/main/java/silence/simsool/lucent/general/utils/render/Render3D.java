...
public class Render3D {
	...
	public static void init() {
		LucentEvent.WORLD_RENDER_LAST.register(event -> {
			...
			renderQueuedCircles(matrix, bufferSource);
	...
	public static void drawText(String text, Vec3 pos, float scale, int color, boolean depth, boolean shadow) {
		Font font = mc.font;
		queuedTexts.add(getOrCreateText(text, pos, scale, depth, UWorld.getCamera().rotation(), font, font.width(text), color, shadow));
	}
	...
	private static void renderQueuedCircles(PoseStack matrix, MultiBufferSource.BufferSource bufferSource) {
		if (queuedCircles.isEmpty()) return;
		PoseStack.Pose last = matrix.last();

		for (CircleData circle : queuedCircles) {
			VertexConsumer buffer = bufferSource.getBuffer(circle.renderType());
			float cx = (float) circle.center.x;
			float cy = (float) circle.center.y;
			float cz = (float) circle.center.z;
			...
			if (circle.filled) {
				...
				for (int i = 0; i < 360; i += 2) {
					...
					buffer.addVertex(last, cx + dx1, cy, cz + dz1).setColor(circle.r, circle.g, circle.b, circle.a).setNormal(last, nx, 0, nz).setLineWidth(circle.thickness);
					buffer.addVertex(last, cx + dx2, cy, cz + dz2).setColor(circle.r, circle.g, circle.b, circle.a).setNormal(last, nx, 0, nz).setLineWidth(circle.thickness);
				}
			}
		}
	}
	...
	private static void renderQueuedTexts(PoseStack matrix, MultiBufferSource.BufferSource bufferSource, Vec3 camera) {
		for (TextData textData : queuedTexts) {
			...
			textData.font.drawInBatch(textData.text, -textData.textWidth / 2f, 0f, textData.color, textData.shadow, pose, bufferSource, textData.depth ? Font.DisplayMode.POLYGON_OFFSET : Font.DisplayMode.SEE_THROUGH, 0, LightTexture.FULL_BRIGHT);
			matrix.popPose();
		}
	}
	...
	private static TextData getOrCreateText(String text, Vec3 pos, float scale, boolean depth, Quaternionf rotation, Font font, float width, int color, boolean shadow) {
		if (textPoolIndex < textPool.size()) {
			TextData data = textPool.get(textPoolIndex++);
			data.set(text, pos, scale, depth, rotation, font, width, color, shadow);
			return data;
		} else {
			TextData data = new TextData(text, pos, scale, depth, rotation, font, width, color, shadow);
			textPool.add(data);
			textPoolIndex++;
			return data;
		}
